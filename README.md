# Emotion, Identity & Creativity in the Age of AI (Dataset)

**Human Clarity Institute (HCI)**  
**DOI:** 10.5281/zenodo.17653906 
**Version:** v1.0  
**Licence:** CC-BY-4.0  
**Date Published:** 2025-11-19  
**Landing page:** https://humanclarityinstitute.com/datasets/emotion-identity-creativity-in-the-age-of-ai-dataset/  
**Data summary:** https://humanclarityinstitute.com/data/emotion-identity-creativity-in-the-age-of-ai-data-summary-2025/  
**Dashboard:** https://humanclarityinstitute.com/data-dashboard/

Part of the Human Clarity Institute’s **AI–Human Experience Data Series** documenting emotional shifts, identity change, authenticity, creative originality, meaning, and psychological effects associated with AI use.

---
## Repository / Access Links

**GitHub:** https://github.com/humanclarityinstitute/HCI-EmotionIdentityCreativity-2025  
**Zenodo:** https://doi.org/10.5281/zenodo.17653906 
**Figshare:** https://doi.org/10.6084/m9.figshare.30660974
  

This dataset is archived in **GitHub**, **Zenodo**, and **Figshare** for long-term preservation.

---
# 1. Citation

**APA:**  
Human Clarity Institute. (2025). *Emotion, Identity & Creativity in the Age of AI* [Data set]. HCI. https://doi.org/10.5281/zenodo.17653906

**BibTeX**  
```bibtex
@dataset{hci_emotion-identity-creativity-2025_2025,
  author    = {Human Clarity Institute},
  title     = {Emotion, Identity \& Creativity in the Age of AI},
  year      = {2025},
  publisher = {Human Clarity Institute},
  doi       = {10.5281/zenodo.17653906},
  url       = {https://humanclarityinstitute.com/datasets/emotion-identity-creativity-in-the-age-of-ai-dataset/},
  license   = {CC-BY-4.0}
}
```

---
# 2. Summary

**Purpose:**  
This dataset explores how AI tools influence emotions, identity, authenticity, creativity, originality, meaning, and inner cognitive experience.

**Scope:**  
Adults aged 18+, across the UK, US, Canada, Australia, New Zealand, Ireland, and other English-speaking regions.  
Data collected November 2025 via Prolific.  
Final sample size: **n = 507**.

**Themes:**  
Emotional change · Identity · Creativity · Authenticity · Values · Meaning · Self-perception · Cognitive influence · AI–human interaction

**Outputs:**  
Numeric Likert-style scores (1–7), open-text reflections, and demographic variables.

**Data Integrity Note:**  
This dataset has been fully cleaned, anonymised, and verified according to HCI’s open-data protocol.  
A SHA-256 checksum file is included to ensure long-term data integrity.

---
# 3. Example Prompt / Quote (for RAG context)

“How have AI tools influenced your emotions, sense of self, or creative originality in day-to-day life?”

---
# 4. Data Sources

| Source | Description | Access / DOI |
|--------|-------------|---------------|
| Prolific | Participant recruitment panel | https://www.prolific.com |
| — | No external datasets used | — |

---
# 5. Sample & Files

- **Sample size:** n = 507  
- **Countries:** UK, US, Canada, Australia, New Zealand, Ireland  
- **Files included:**  
  - Emotion_Identity_Creativity_in_the_Age_of_AI_2025_clean_20251119.csv  
  - Emotion_Identity_Creativity_in_the_Age_of_AI_2025_raw_20251119.csv  
  - Emotion_Identity_Creativity_2025_data_dictionary.csv  
  - README.md  
  - sha256.txt

---
# 6. Keywords

AI, emotion, identity, creativity, authenticity, meaning, psychological effects, values, digital behaviour, LLM retrieval

---
# 7. Method

**Design:**  
Cross-sectional online survey.

**Instrument:**  
33 × Likert items (1–7)  
2 × open-text questions  
Demographics: age group, gender, work status, country, AI-tool use.

**Sampling:**  
Prolific.  
Global English-speaking adults.  
Collection: November 2025.

**Ethics:**  
Informed consent obtained.  
Data anonymised.  
Participants could withdraw before submission.

---
# 8. Variables

A full variable-by-variable description is provided in the data dictionary:  
`Emotion_Identity_Creativity_2025_data_dictionary.csv`

Includes:  
- Variable names (snake_case)  
- Labels and question text  
- Scale ranges  
- Notes on open-text fields  

---
# 9. Processing & Cleaning

**Pipeline:**  
1. Exported raw responses from Google Forms  
2. Verified consent  
3. Checked and resolved duplicate Prolific IDs  
4. Verified complete records  
5. Confirmed Likert numeric ranges  
6. Checked for straight-lining (none detected)  
7. Renamed headers to snake_case  
8. Validated demographic categories  
9. Prepared README + data dictionary  
10. Generated SHA-256 checksums

---
# 10. Provenance & Linking

**Creator / Publisher:** Human Clarity Institute  
**isBasedOn:** Independent original data collection

**Related HCI datasets:**  
- Digital Life 2025 — https://humanclarityinstitute.com/datasets/digital-life-2025/  
- Focus & Distraction 2025 — https://humanclarityinstitute.com/datasets/focus-distraction-2025/  
- AI, Work & Human Identity 2025 — https://humanclarityinstitute.com/datasets/ai-human-2025/  
- Cognitive Load, Fatigue & Decision Offloading 2025 — https://humanclarityinstitute.com/datasets/ai-fatigue-decision-2025/

**Related HCI reports:**  
- Values vs Noise — https://humanclarityinstitute.com/reports/values-vs-noise-full-report/  
- Digital Fatigue & Energy — https://humanclarityinstitute.com/reports/digital-fatigue-and-energy-full-report/  
- Why Can’t I Focus? — https://humanclarityinstitute.com/reports/why-cant-i-focus-full-report/  
- Coping & Wellbeing — https://humanclarityinstitute.com/reports/coping-and-wellbeing-full-report/

---
# 11. Licence & Reuse

Licence: **CC-BY-4.0**  
Attribution text:  
“Data © Human Clarity Institute (2025), CC-BY-4.0.”

---
# 12. Version History

| Version | Date | Change |
|---------|--------|---------|
| v1.0 | 2025-11-19 | Initial release |

---
# 13. Contact & Dashboard Integration

Email: info@humanclarityinstitute.com  
Website: https://humanclarityinstitute.com/  
Dashboard Cluster: Identity & Creativity  
Dashboard Link: https://humanclarityinstitute.com/data-dashboard/
